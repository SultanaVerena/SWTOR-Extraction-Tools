...
def create_materials(filepath, relpath, unique_materials, use_cycles):
    """
    ...
    for name in unique_materials:
        if name is not None:
            ...
            if use_cycles:
                ...
                mat_wrap = cycles_shader_compat.CyclesShaderWrapper(mat)
                cycles_material_wrap_map[mat] = mat_wrap
    ...
    with ProgressReport(context.window_manager) as progress:
        ...
        with open(filepath, 'rb') as f:
            ...
            for mesh in range(0, num_meshes):
                ...
                if vert_size not in [12, 24, 32, 36]:
                    operator.report({'ERROR'},
                                    "\'%s\' cannot be loaded as it uses vertices with a size of %i bytes. \n"
                                    "This addon only supports files that use vertex sizes of 12, 24, 32 or 36 bytes."
                                    % (filepath, vert_size))
                    return {'CANCELLED'}
                ...
                if vert_size != 12:
                    ...
                    for piece in range(0, num_pieces):
                        ...
                        f.seek(36, 1)  # Mesh piece enumerator (1 x uint32) & bounding box (8 x 4 bytes)
                        cur_off = f.tell()
                        ...
                        if piece_mat_id == 4294967295 and piece == 0:
                            ...
                            context_material = mesh_name + "_" + "{:03d}".format(piece)  # Use mesh name.00x for material
                        else:
        ...
        create_materials(filepath, relpath, unique_materials, use_cycles)
